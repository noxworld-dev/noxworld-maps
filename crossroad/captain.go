...
func dialogExampleCaptain() {
	...
	// Warrior
	// War01A.scr:CaptainTalkStart : Gain entry to Dün Mir, find the Academy and then be ready for the test of your life -- The Gauntlet. It weeds out the weak recruits.
	...
	if rnd == 2 {
		...
		for i := 0; i < len(wizardClass); i++ {
			if ns.GetCaller() == wizardClass[i] {
				ns.SetDialog(captain, ns.DialogNext, captainDialogueAdviceWizard, captainDialogueAdviceWizard)
...
func captainDialogueAdviceWizard() {
	ns.SetDialog(captain, ns.DialogNormal, captainDialogueAdviceWizard, captainDialogueEnd)
