...
func checkIfNearbyExit() {
	...
	if ns.Object("MinesToCrossRoads") != nil {
		if (ns.InCirclef{Center: ns.Object("MinesToCrossRoads"), R: 50}).Matches(ns.GetHost()) {
			exitUsed = true
			updateNoxWorldData(ns.GetHost().Player(), func(data *NoxWorldData) {
				data.Character.LastExitUsed = "MinesExitToCrossRoads"
				ns.LoadMap("nw_xroad", &ns.LoadMapOptions{HideTitleScreen: true})
			})
		}
	}
	...
	if ns.Object("Galava01ToGalava02") != nil {
		if (ns.InCirclef{Center: ns.Object("Galava01ToGalava02"), R: 50}).Matches(ns.GetHost()) {
			exitUsed = true
			updateNoxWorldData(ns.GetHost().Player(), func(data *NoxWorldData) {
				data.Character.LastExitUsed = "Galava01ExitToGalava02"
				ns.LoadMap("nw_galava02", &ns.LoadMapOptions{HideTitleScreen: true})
			})
		}
	}
	if ns.Object("Galava02ToGalava01") != nil {
		if (ns.InCirclef{Center: ns.Object("Galava02ToGalava01"), R: 50}).Matches(ns.GetHost()) {
			exitUsed = true
			updateNoxWorldData(ns.GetHost().Player(), func(data *NoxWorldData) {
				data.Character.LastExitUsed = "Galava02ExitToGalava01"
				ns.LoadMap("nw_galava01", &ns.LoadMapOptions{HideTitleScreen: true})
			})
		}
	}
...
func moveToEntry() {
	...
	if data.Character.LastExitUsed == "CrossRoadsExitToMines" {
		for i := 0; i < len(ns.Players()); i++ {
			ns.Players()[i].Unit().SetPos(ns.Waypoint("MinesEntryFromCrossRoads").Pos())
		}
	}
	if data.Character.LastExitUsed == "Galava02ExitToGalava01" {
		for i := 0; i < len(ns.Players()); i++ {
			ns.Players()[i].Unit().SetPos(ns.Waypoint("Galava01EntryFromGalava02").Pos())
		}
	}
}
